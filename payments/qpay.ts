...
export class Qpay {
	...
	private saveToken: (token: TokenResponse) => Promise<void>;
	...
	private async checkToken(token?: TokenResponse) {
		if (!token) {
	...
	async createInvoice(input: CreateInvoiceRequestInput, token?: TokenResponse) {
	...
	async checkInvoice(invoiceId: string, token?: TokenResponse) {
